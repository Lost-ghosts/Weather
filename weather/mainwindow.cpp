    ...
    , ui(new Ui::MainWindow)
{
    ...
    // ????????API???
        ...
        sunTimer->start(1000);
        connect(sunTimer,SIGNAL(timeout()),ui->sunRiseSetLb,SLOT(updata()));
...
{
    QJsonParseError err;
    QJsonDocument jsonDoc = QJsonDocument::fromJson(bytes,&err);    //????json???
    if(err.error != QJsonParseError::NoError)       //Json???????
    {
        return;
    }
...
void MainWindow::paintCurve()
{
    ...
    //算出温度对应坐标
    int pointX[6] = {36,104,173,242,311,380};   //点的X坐标
    int pointHY[6] = {0};
    int pointLY[6] = {0};
    for(int i = 0;i < 6;i++)
    {
